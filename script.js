...
const GRADE_SYSTEMS = [
    ...
    { title: 'الثانوية العامة', grades: [{ name: 'الصف الأول الثانوي', val: 10, num: 1 }, { name: 'الثاني الثانوي', val: 11, num: 2 }, { name: 'الثالث الثانوي', val: 12, num: 3 }, { name: 'علمي علوم', val: 13, num: 4 }, { name: 'علمي رياضة', val: 14, num: 5 }, { name: 'القسم الأدبي', val: 15, num: 6 }] },
    ...
    { title: 'البكالوريا', grades: [{ name: 'البكالوريا', val: 17, num: 1 }] }
];
...
window.injectBookTransition = function () {
    ...
    loader.innerHTML = `
        ...
        <div style="position:absolute; font-family:'Tajawal'; color:#fff; font-size:40px; font-weight:900; z-index:2; transition: opacity 0.5s;" class="book-logo">لسان العرب</div>
    `;
...
window.injectCountryModal = function () {
    ...
    modal.innerHTML = `
        <div style="background:#fff; width:90%; max-width:600px; border-radius:30px; padding:40px; position:relative; box-shadow:0 30px 60px rgba(0,0,0,0.5); text-align:center;">
            ...
            <h2 style="font-size:32px; font-weight:900; color:#132644; margin-bottom:30px;">اختر الدولة <span style="color:#12b8c5;">لسان العرب</span></h2>
...
window.sendWhatsApp = function (msg) {
    let finalMsg = "أهلاً بك في منصة لسان العرب التعليمية.\n\n" + msg;
...
window.requestCode = function () {
    let msg = "أريد طلب كود اشتراك جديد لمنصة لسان العرب التعليمية";
    ...
    if (!document.getElementById('my-subs-login-modal')) {
        ...
        modal.innerHTML = `
            <div style="background:#fff; border-radius:20px; padding:30px; width:90%; max-width:500px; text-align:center; box-shadow:0 20px 50px rgba(0,0,0,0.2);">
                ...
                <a href="#" onclick="window.sendWhatsApp('أهلاً، نسيت كود الاشتراك الخاص بي لمنصة لسان العرب، هل يمكنكم مساعدتي؟'); return false;" style="color:#f44336; font-weight:bold; text-decoration:underline;">هل نسيت الكود؟</a>
...
window.registerAttendance = async function () {
    ...
    alert("ممتاز! تم تسجيل حضورك بنجاح في سجلات منصة لسان العرب للإدارة 🎓");
}
...
window.promptAdmin = async function () {
    let pass = prompt("بوابة لسان العرب للإدارة - يرجى كتابة الرقم السري (الأساسي أو المساعد):");
...
window.injectWhatsAppButton = function () {
    ...
    btn.href = 'https://wa.me/201281682711?text=' + encodeURIComponent('أهلاً بك في منصة لسان العرب التعليمية.\\n');
    btn.target = '_blank';
